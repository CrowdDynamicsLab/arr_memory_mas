...
Tables~\ref{tab:app_e2_rho_gpt} and~\ref{tab:app_e2_rho_ds} give the full $\sigma$-vs-$\rho$ trajectories for both models. 
\texttt{qwen3:32b} was run only on the hard-budget E2 conditions and is therefore reported in the main compression table rather than the full trajectory tables.
...
\texttt{qwen3:32b}'s clean allowlist leaks slightly more than GPT-5-mini or \texttt{deepseek-R1-32B} (4/48 vs.\ 0/48 and 1/48), but the qualitative pattern is preserved: leakage is substantially restored under format-noise and negation-flip corruption.
...
\begin{tabular}{@{}lccc@{}}
\toprule
\textbf{Variant} & \textbf{GPT-5-mini} & \textbf{DeepSeek-r1} & \textbf{Qwen3} \\
\midrule
...
\paragraph{Fisher exact tests.} Table~\ref{tab:app_fisher} reports two-sided Fisher exact $p$-values for the primary \texttt{gpt-5-mini} and \texttt{deepseek-R1-32B} contrasts.
...
\midrule
E3 dump+marker vs.\ dump-no-marker & \texttt{gpt-5-mini}      & $1.61 \times 10^{-12}$ \\
E3 dump+marker vs.\ dump-no-marker & \texttt{deepseek-R1-32B} & $1.98 \times 10^{-13}$ \\
E8 L1 vague vs.\ L2 suggestion     & \texttt{gpt-5-mini}      & $2.13 \times 10^{-9}$  \\
E8 L1 vague vs.\ L2 suggestion     & \texttt{deepseek-R1-32B} & $4.02 \times 10^{-5}$  \\
E9 typed allowlist vs.\ E6 overall & \texttt{gpt-5-mini}     & $1.04 \times 10^{-4}$  \\
E9 typed allowlist vs.\ E6 overall & \texttt{deepseek-R1-32B} & $1.58 \times 10^{-3}$  \\
\bottomrule
\end{tabular}
\end{table*}
...
\midrule
original        & \texttt{gpt-5-mini}      & 0.690 & 0.997 & $-0.306$ \\
original        & \texttt{deepseek-R1-32B} & 0.580 & 0.972 & $-0.392$ \\
binary strict   & \texttt{gpt-5-mini}      & 0.722 & 1.000 & $-0.278$ \\
binary strict   & \texttt{deepseek-R1-32B} & 0.611 & 0.972 & $-0.361$ \\
binary lenient  & \texttt{gpt-5-mini}      & 0.750 & 1.000 & $-0.250$ \\
binary lenient  & \texttt{deepseek-R1-32B} & 0.681 & 1.000 & $-0.319$ \\
\bottomrule
\end{tabular}
\end{table}
% =====================================================================
