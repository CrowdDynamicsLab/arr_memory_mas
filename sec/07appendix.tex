...
\begin{tabular}{@{}lcccc@{}}
\toprule
\textbf{Surface} & \multicolumn{2}{c}{\textbf{gpt-5-mini}} & \multicolumn{2}{c}{\textbf{DS-r1:32b}} \\
\cmidrule(lr){2-3}\cmidrule(l){4-5}
...
\midrule
clean allowlist                   & 0 / 48  (0.0\%)  & 1 / 48  (2.1\%)  \\
dump-with-marker control          & 0 / 48  (0.0\%)  & 1 / 48  (2.1\%)  \\
\midrule
corrupt: scope drift              & 10 / 48 (20.8\%) & 4 / 48  (8.3\%)  \\
corrupt: typo allowlist key       & 23 / 48 (47.9\%) & 18 / 48 (37.5\%) \\
corrupt: format noise             & 40 / 48 (83.3\%) & 22 / 48 (45.8\%) \\
corrupt: negation flip            & 44 / 48 (91.7\%) & 38 / 48 (79.2\%) \\
\midrule
dump-no-marker control            & 43 / 48 (89.6\%) & 39 / 48 (81.2\%) \\
\bottomrule
\end{tabular}
\end{table}
...
The compressed Related Work in the main paper groups prior work into four threads. This appendix expands the positioning relative to adjacent literature.

\paragraph{Privacy benchmarks vs.\ the handoff artifact.}
ConfAIde~\citep{mireshghallah2023can}, PrivacyLens~\citep{shao2024privacylens}, and AgentDAM~\citep{zharmagambetov2025agentdam} treat the agent as a single decision-making unit and ask whether a final action respects a contextual disclosure norm. AgentDAM additionally operationalizes data minimization for web agents. Our unit of analysis is different: the cross-phase summary produced when one agent compresses upstream interaction for another. In that setting, the norm violation can originate before the final speaker acts --- a summarizer can preserve the sensitive fact while silently dropping the marker that constrained how the fact could be used.

\paragraph{Multi-agent leakage surfaces.}
AgentLeak~\citep{yagoubi2026agentleak} shows that violations frequently occur in internal inter-agent channels rather than user-facing outputs. SumLeak~\citep{patil2025sum} formalizes compositional leakage in which individually innocuous responses aggregate into a sensitive attribute. Prompt Infection~\citep{lee2024prompt}, MASLeak~\citep{wang2025ip}, and Agent-in-the-Middle~\citep{he2025red} demonstrate adversarial inter-agent attacks. Our setting is non-adversarial: the sensitive information is already present at the source and is often accompanied by an explicit boundary marker that should constrain its use; the failure is that the marker, not the fact, is weakened or dropped in cross-phase compression.

\paragraph{Memory extraction.}
MEXTRA~\citep{wang2025unveiling} demonstrates black-box extraction of private user--agent interactions stored for future demonstrations. We adopt the same channel-level lens but study a non-adversarial failure: when one agent's output becomes another's input via summarization, what governance information is preserved, what is silently lost, and when does that loss become final audience-facing leakage?

\paragraph{Summarization fidelity.}
Outside the agent literature, abstractive-summarization work has long observed that compression drops hedges, attributions, and conditions that qualify what surface-level facts mean~\citep{maynez2020faithfulness}. Our finding is the multi-agent privacy analogue: the propositions most vulnerable to compression are often those that license or constrain the propositions that survive. The downstream agent then receives a fact without the rule that made the fact safe to use.

\paragraph{Communication Privacy Management.}
CPM~\citep{petronio1991communication, petronio2002boundaries, griffin2006first} models privacy as boundary work: individuals own private information, license its disclosure to specific co-owners under specific use conditions, and experience boundary turbulence when those conditions are violated. CPM provides a vocabulary that distinguishes the operational content of a fact from the rule that governs its use. Our contribution, in CPM terms, is to identify a structural failure of MAS --- the cross-phase summary --- in which operational content consistently survives while the licensing rule consistently does not.

\section{Extended Ethics Discussion}
\label{app:ethics}

This appendix expands the brief Ethics Statement in the main paper.
...
\paragraph{Dual-use considerations.} A paper that catalogues how multi-agent systems leak private information could in principle inform an adversary. We judge the net effect of disclosure to be positive: (i) summary collapse is a structural property of how current LLMs compress dialogue, not a novel attack vector; (ii) the failure mode is already implicitly exploited any time an automated handoff system propagates context dump; and (iii) the operational-lifting probe points to a constructive direction practitioners can adopt before the gap is closed in models themselves. Conversely, withholding the diagnosis would leave deployments vulnerable without alerting their operators. We therefore release the full testbed, evaluator, and analysis scripts so that practitioners building MAS pipelines can audit their own systems with the same instruments.

\paragraph{Model and compute use.} Experiments use \texttt{gpt-5-mini} through the OpenAI API and \texttt{deepseek-r1:32b} through a campus inference endpoint at temperature 0.1. We do not fine-tune any model and do not train new model weights, so the carbon and compute footprint of our work consists solely of inference. No human subjects participated in the experiments themselves; the manual labeling was conducted by the authors and treated as a methodological audit, not as a labeled-data collection.
