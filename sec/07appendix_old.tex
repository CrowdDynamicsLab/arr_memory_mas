...
\begin{table}[h]
\centering
\caption{Per-domain manifest of the 36-scenario \textsc{Bound-Handoff} benchmark. ``Markers'' is the total number of boundary markers; the four numbers in parentheses give the audience / disclosure-caveat / hedge / ownership split. ``Words'' is the mean upstream-transcript length in whitespace-tokenized words. Source: \texttt{dataset/data/bound\_handoff\_phase2.json}.}
...
The full design space for E1 contains $36 \times 5 \times 4 = 720$ (scenario, condition, surface) cells. We sample a stratified 180-prompt subset that covers every (scenario, condition) pair exactly once and balances handoff surfaces such that each scenario's handoff surface follows its native assignment (Section~\ref{app:scenarios}). The stratification is deterministic given the scenario manifest; the construction script is \texttt{build\_handoff\_prompts.py}. The prompt manifest used in our experiments is \texttt{dataset/data/bound\_handoff\_phase2\_prompts.jsonl}.

\subsection{Judge Prompt Template}
\label{app:judge}

Both $\sigma$ and $\sigma_{\mathrm{op}}$ are scored by an LLM judge using a single template that returns JSON. The template is reproduced in Figure~\ref{fig:judge_prompt}; the full implementation including retry and JSON-validation logic is in \texttt{dataset/scripts/judge\_phase2\_sigma.py}.
...
\bottomrule
\end{tabular}
\caption{Judge prompt template for $\sigma$ and $\sigma_{\mathrm{op}}$. Verbatim source in \texttt{dataset/scripts/judge\_phase2\_sigma.py}.}
...
\begin{table}[h]
\centering
\caption{E1 baseline survival by handoff surface on \texttt{deepseek-r1:32b}. Source: \texttt{summary\_uiuc\_deepseek\_phase2\_sigma\_by\_surface.csv}.}
...
\begin{table}[h]
\centering
\caption{E1 baseline boundary survival by marker category on both models. Sources: \texttt{summary\_*\_phase2\_sigma\_by\_marker\_category.csv}.}
...
\begin{table}[h]
\centering
\caption{E1 baseline survival by domain on both models. Sources: \texttt{summary\_*\_phase2\_sigma\_by\_domain.csv}.}
...
\begin{table}[h]
\centering
\caption{Full $\sigma$-vs-$\rho$ trajectory on \texttt{gpt-5-mini}. Source: \texttt{sigma\_vs\_rho\_by\_condition.csv}.}
...
\begin{table}[h]
\centering
\caption{Full $\sigma$-vs-$\rho$ trajectory on \texttt{deepseek-r1:32b}. Source: \texttt{sigma\_vs\_rho\_by\_condition\_deepseek.csv}.}
...
\begin{table}[h]
\centering
\caption{Survival by handoff surface under 25-word compression on both models. Source: \texttt{summary\_*\_phase2\_stresstest\_hard\_budget\_v2\_full36\_sigma\_by\_surface\_20260512.csv}.}
...
\begin{table*}[h]
\centering
\caption{E3 final leakage by downstream pressure. Cells report leakage / total. Source: \texttt{phase2b\_model\_comparison.csv}.}
...
Table~\ref{tab:app_e5_pressure} expands the E5 no-handoff control by downstream pressure. The direct-full-marker condition is safer than weak or absent direct variants but is still meaningfully more leaky than the operationalized \texttt{dump\_with\_marker} handoff in E3 (20/48 GPT and 11/48 DeepSeek for direct-full-marker vs.\ 2/48 and 1/48 for \texttt{dump\_with\_marker}). This is the empirical content of the Section~\ref{sec:results_e5} claim that the load-bearing axis is whether the boundary is operationalized, not whether the system uses one or two agents.

\begin{table*}[h]
\centering
\caption{E5 no-handoff single-agent control by pressure. Cells report exact-phrase leakage / total. Source: \texttt{eval\_*\_single\_agent\_control\_calibrated.jsonl}.}
...
Table~\ref{tab:app_e6_pressure} gives the per-condition $\times$ per-pressure leakage matrix that is referenced but not shown in Section~\ref{sec:results_e6e7}. The two structured-schema variants leak under the audit-trace and compressed-report pressures on both models, even though the \texttt{structured\_schema\_safe} variant is the format intended as a privacy mitigation. DeepSeek's \texttt{minimality\_instruction} condition is the single zero-leak cell in the table.

\begin{table*}[h]
\centering
\caption{E6 final leakage by mitigation condition $\times$ downstream pressure. Cells report leakage / 12. Sources: \texttt{eval\_openai\_gpt5mini\_exp5\_mitigation\_downstream\_calibrated.jsonl}, \texttt{eval\_uiuc\_deepseek\_exp5\_mitigation\_downstream\_calibrated.jsonl}.}
...
\begin{table}[h]
\centering
\caption{E7 redaction counts before downstream evaluation. Source: derived from \texttt{model\_outputs\_openai\_gpt5mini\_exp5b\_mitigation\_handoffs\_redacted.jsonl}.}
...
\begin{table}[h]
\centering
\caption{E7 high-risk manual audit on both models. Sources: \texttt{audit\_openai\_gpt5mini\_exp5b\_manual\_sample40\_labeled.jsonl}, \texttt{manual\_audit\_uiuc\_deepseek\_exp5b\_highrisk\_summary.csv}.}
...
\begin{table}[h]
\centering
\caption{DeepSeek E7 high-risk manual audit by domain. Source: \texttt{manual\_audit\_uiuc\_deepseek\_exp5b\_highrisk\_summary.csv}.}
...
Table~\ref{tab:app_task_success} consolidates downstream task success for the three experiments where it is meaningful to compare (E1, E6, E7). Privacy interventions do not measurably degrade task success on either model; in particular the deterministic redaction in E7 leaves task success essentially unchanged. Sources: \texttt{summary\_task\_success\_*\_\{phase2,exp5,exp5b\}.csv}.
