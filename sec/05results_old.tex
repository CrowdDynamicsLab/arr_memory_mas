...
Phase~2B context-dump test & 12 scenarios $\times$ 3 variants $\times$ 4 pressures per model & Final audience-facing leakage & No dump: 0/48 on both models; dump with explicit marker: 2/48 GPT, 1/48 DeepSeek; dump without marker: 48/48 GPT, 37/48 DeepSeek \\
...
Table~\ref{tab:phase2b_main} shows that the three variants separate cleanly. Data minimization is fully protective in the tested cells: no-dump variants produce 0/48 leaks on both models. Explicit markers are also strongly protective: leakage is 2/48 on GPT and 1/48 on DeepSeek. Without a marker, dumped sensitive context becomes the modal downstream behavior: 48/48 on GPT and 37/48 on DeepSeek.
